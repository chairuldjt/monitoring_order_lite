...
import { Search, RefreshCw, Wifi, ChevronDown, X, ArrowUp, ArrowDown, Phone, ArrowRight, FileSpreadsheet, Copy } from 'lucide-react';
...
Lokasi    : ${order.requester_name ? order.requester_name + ' - ' : ''}${order.requester_unit || '-'}
Ext Telp  : ${order.ext_phone || '-'}
Catatan   : ${order.title || order.description || '-'}
Petugas   : ${technicians || '-'}
...
Maintenance: ${isMaintenance}`;
    ...
    const searchOptions = [
        ...
        { key: 'service', label: 'Layanan (Service)' },
    ...
    return (
        <div className="min-h-screen p-4 md:p-8 space-y-6 animate-fade-in relative pb-20">
            ...
            {search && (
                <div className="bg-indigo-50/50 border border-indigo-100 p-4 rounded-2xl flex items-center justify-between gap-4 animate-fade-in-up">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-[10px] md:text-sm font-black text-slate-800 tracking-widest uppercase">Pencarian Aktif</h3>
                            <p className="text-[10px] text-slate-500 font-medium">"{search}" ({total} hasil)</p>
                        </div>
                    </div>
                    <button 
            ...
            <div className="flex gap-2 pb-2 overflow-x-auto no-scrollbar scroll-smooth flex-nowrap -mx-4 px-4 md:mx-0 md:px-0 md:flex-wrap">
                {statuses.map(s => (
                    <button
                        ...
                        className={`px-6 py-2.5 rounded-xl text-[9px] font-black transition-all border uppercase tracking-widest whitespace-nowrap ${statusFilter === s.key
                    ...
                    >
                        ...
                        {summary && summary[s.key || 'all'] ? (
                            <span className={`ml-2 px-1.5 py-0.5 rounded-md text-[8px] ${statusFilter === s.key ? 'bg-violet-400' : 'bg-slate-100'}`}>
                                {summary[s.key || 'all']}
                            </span>
                        ) : null}
                    </button>
            ...
            <div className="bg-white/60 backdrop-blur-md rounded-[2rem] md:rounded-[2.5rem] border border-white/20 shadow-xl overflow-hidden min-h-[400px]">
                {loading ? (
                    <div className="p-10 md:p-20 flex flex-col items-center justify-center text-center">
                        <div className="w-12 md:w-16 h-12 md:h-16 bg-slate-50 rounded-2xl flex items-center justify-center mb-4 animate-pulse">
                            <RefreshCw className="w-6 md:w-8 h-6 md:h-8 text-slate-300 animate-spin" />
                        </div>
                        <h3 className="text-sm md:text-lg font-black text-slate-800 uppercase tracking-widest">Memuat Data...</h3>
                    </div>
                ) : orders.length === 0 ? (
                    <div className="p-10 md:p-20 flex flex-col items-center justify-center text-center">
                        <div className="w-12 md:w-16 h-12 md:h-16 bg-slate-50 rounded-2xl flex items-center justify-center mb-4 text-slate-300">
                            <X className="w-6 md:w-8 h-6 md:h-8" />
                        </div>
                        <h3 className="text-sm md:text-lg font-black text-slate-800 uppercase tracking-widest">Tidak Ada Data</h3>
                        <p className="text-xs md:text-sm text-slate-500 font-medium">Coba sesuaikan filter pencarian atau tanggal.</p>
                    </div>
                ) : (
                    <>
                        {/* Mobile Grid (Block on small, hidden on md+) */}
                        <div className="block md:hidden divide-y divide-slate-100">
                            {orders.map((order) => (
                                <div key={order.order_id} className="p-4 space-y-4 active:bg-violet-50/50 transition-colors">
                                    <div className="flex justify-between items-start">
                                        <div className="flex flex-col">
                                            <span className="text-[10px] font-black text-slate-800 uppercase tracking-wider">{order.order_no}</span>
                                            <span className="text-[9px] text-slate-400 font-bold uppercase">{order.create_date}</span>
                                        </div>
                                        <span className={`px-2 py-0.5 rounded-lg text-[8px] font-black uppercase tracking-widest border ${getStatusColor(order.status)}`}>
                                            {order.status}
                                        </span>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex flex-col gap-1 mt-3">
                                            <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest truncate">{order.requester_name}</span>
                                            <span className="text-[9px] text-violet-500 font-bold uppercase tracking-wider truncate">{order.requester_unit}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between pt-2">
                                        ...
                                        <div className="flex gap-2">
                                            <button 
                                                ...
                                                className="p-2 bg-slate-50 text-slate-400 rounded-lg border border-slate-100 active:scale-95"
                                            >
                                                <Copy className="w-3.5 h-3.5" />
                                            </button>
                                            <Link 
                                                href={`/orders/${order.order_id}`}
                                                className="px-4 py-2 bg-slate-900 text-white rounded-lg text-[9px] font-black uppercase tracking-widest active:scale-95"
                                            >
                                                Detail
                                            </Link>
                                        </div>
                        ...
                        {/* Desktop Table (Hidden on small, block on md+) */}
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-5 align-top">
                                            <div className="flex flex-col">
                                                ...
                                                {order.service_name && (
                                                    <div className="mt-2">
                                                        <span className="px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded-md text-[9px] font-black uppercase tracking-widest border border-indigo-100 block w-fit">
                                                            {order.service_name}
                                                        </span>
                                                    </div>
                                                )}
                                        ...
                                        <td className="px-6 py-5 align-top">
                                            <div className="flex flex-col gap-2">
                                                <Link 
                                                    href={`/orders/${order.order_id}`}
                                                    className="inline-flex items-center justify-center gap-2 bg-white border border-slate-200 text-slate-700 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-800 hover:text-white hover:border-slate-800 transition-all shadow-sm active:scale-95 group/btn"
                                                >
                                                    Detail
                                                ...
                                                <button 
                                                    onClick={() => copyToClipboard(order)}
                                                    className="inline-flex items-center justify-center gap-2 bg-slate-50 border border-slate-200 text-slate-500 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 hover:text-slate-700 transition-all shadow-sm active:scale-95"
                                                >
